...
#if defined(CFG_MX6) || defined(CFG_MX7)
#include "imx-regs.h"
#else
#endif
...
#define RTMCTL_PRGM 0x00010000  // 1 -> program mode, 0 -> run mode
				// use von Neumann data in both entropy shifter
...
#define sec_write_addr(a, v) sec_out32(ptov((a)), (v))
#endif
...
#define SET_JR_REG(name, jr, val) \
	(sec_out32(ptov(JR_REG(name, (jr))), (val)))

#define SET_JR_REG_LO(name, jr, val) \
	(sec_out32(ptov(JR_REG_LO(name, (jr))), (val)))

// STRUCTURES AND OTHER TYPEDEFS
// Lists the possible states for a job ring.
enum sec_job_ring_state_e {
...
struct sec_job_ring_t {
	...
	enum sec_job_ring_state_e jr_state;
};
...
static inline void hw_enqueue_desc_on_job_ring(struct jr_regs *regs, int num)
{
	sec_out32(ptov(&regs->irja), num);
}
