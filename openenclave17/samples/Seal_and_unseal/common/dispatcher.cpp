    ...
    int seal_size)
{
    ...
    m_crypto->get_aes_key(m_aes_key);
    // string message = rand_str(seal_size);
    if ((requests_message_size + state_message.size()) > sizeof(data))
    {
        TRACE_ENCLAVE("Encrypt data buffer is more small!!");
        goto exit;
    }
    memcpy(data, requests_message, requests_message_size);
    memcpy(data + requests_message_size, state_message.c_str(), state_message.size());
    ...
    size_t *is_ready)
{
    int ret = 1;
    string message;
    uint8_t decrypt_data[1024]; // TODO maybe change buffer to pointer
 ...
 */
// TODO
int ecall_dispatcher::unseal_state_data(unsigned char *sealed_data, size_t sealed_data_size)
{
    uint8_t *temp_data;
    int ret = 1;
    const char *state = "test plaintext";
    unsigned char *optional_message = (unsigned char *)state;
    size_t optional_message_size = strlen((const char *)state);
    oe_result_t result;
    ...
    if (result != OE_OK)
    {
        TRACE_ENCLAVE("oe_unseal() failed with %d %s\n", ret, oe_result_str(result));
        ret = 1;
        return ret;
    }
    state_info_table2 = (state_info_t *)malloc(sizeof(state_info_t) + temp_data_size);
    memset(state_info_table2, 0, sizeof(sizeof(state_info_t) + temp_data_size));
    memcpy(state_info_table2, temp_data, temp_data_size);
    // TRACE_ENCLAVE("oe_unseal() successful");
    ret = 0;
exit:
    oe_free(temp_data);
    oe_free(state_info_table2);
    state_info_table2 = NULL; // TODO
    temp_data = NULL;
    state = NULL;
    ...
    uint8_t *ITHash)
{
    ...
    if ((requests_message_size > sizeof(state_info_table->requests_I)))
    {
        TRACE_ENCLAVE("Size is more bigger than table.You size is %d and %d", requests_message_size, status_message.size());
        goto exit;
    }
    // checkpoint every 100 times seal its message
    state_info_table = (state_info_t *)malloc(sizeof(state_info_t) + status_message.size() + 128);
    memset(state_info_table, 0, sizeof(state_info_t) + status_message.size() + 128);
    memcpy(state_info_table->state, status_message.c_str(), status_message.size());
    state_info_table->state_size = status_message.size();
    table_k = sizeof(state_info_t) + status_message.size();
    ...
    // OCall
    // TRACE_ENCLAVE("The seal size is %zu ", blob_size);
    Ocall_buffer = (unsigned char *)oe_host_malloc(blob_size + 1);
    memcpy(Ocall_buffer, blob, blob_size);

    result = seal_host_write(&ret, Ocall_buffer, blob_size);
    if (result != OE_OK)
    {
...
int ecall_dispatcher::updateITHash(
    uint8_t *reply_data,
    size_t reply_data_size)
{
    ...
    memcpy(state_info_table->ITHash, tmpITHash, 32); // save inside the state info table
    ret = 0;
    TRACE_ENCLAVE("AE accept its ITHash and checkpoint count ++ %zu", decrypt_data_size);
    // checkpoint_count++;
exit:
    return ret;
}