    ...
    size_t data_size)
{
    ...
    if (PRINT_ATTESTATION_MESSAGES)
    {
        TRACE_ENCLAVE("Attestation Info: executing attestation size: %lu.\n", evidence_size);
    }
    ...
    // //other_signer_id is ower se
    // if (memcmp(claim->value, other_signer_id, OE_UNIQUE_ID_SIZE) != 0)
    // {
    //     TRACE_ENCLAVE("Error: signer_id checking failed");

    //     for (int j = 0; j < OE_UNIQUE_ID_SIZE; j++)
    //     {
    //         TRACE_ENCLAVE("Error: print my SIGNER_ID [%d]=0x%0x", j, (uint8_t)m_enclave_signer_id[j]);
    //     }
    //     TRACE_ENCLAVE("\n");
    //     for (int j = 0; j < OE_UNIQUE_ID_SIZE; j++)
    //     {
    //         TRACE_ENCLAVE("Error: print receiving claim[%d]=0x%0x", j, (uint8_t)claim->value[j]);
    //     }
    //     goto exit;
    // }
