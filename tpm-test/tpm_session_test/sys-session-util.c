...
#include "tpm_test_util/tss2_endian.h"
    ...
    TSS2_TCTI_CONTEXT *tctiContext)
{
    ...
    LOG_INFO("session->encryptedSalt.size = %d", session->encryptedSalt.size);
    ...
    if (rval != TPM2_RC_SUCCESS) {
        LOG_ERROR("StartAuthSession failed. detail: %s", Tss2_RC_Decode(rval));
        goto out;
    }
    ...
    LOG_INFO("after authvalue, key.size = %d", key.size);
    if (rval != TPM2_RC_SUCCESS) {
    ...
    LOG_INFO("after salt, key.size = %d", key.size);
    ...
    LOG_INFO("sessionKey.size = %d", session->sessionKey.size);
out:
    ...
    TSS2L_SYS_AUTH_COMMAND *pSessionsDataIn)
{
    ...
    for (i = 0; i < count; i++) {
        LOG_INFO("compute_command_hmac, i = %d", i);
        ...
        LOG_INFO("hmac_key.size = %d", hmac_key.size);
...
{
    // LOG_INFO("new_nonce.size = %d", new_nonce->size);
    // for (int i = 0; i < new_nonce->size; i++) {
    //     LOG_INFO("nonce.buffer[i] = %02x", new_nonce->buffer[i]);
    // }
    ...
    TPM2B_DIGEST *pHash)
{
    ...
    if (command) {
        ...
        if (rval != TPM2_RC_SUCCESS)
                return rval;
        // LOG_INFO("name1.size = %d", name1.size);
        // for (int ii = 0; ii < name1.size; ii++) {
        //     LOG_INFO("name1.byte-%d: %02x", ii, name1.name[ii]);
        // }
        rval = tpm_handle_to_name(tcti_context, handle2, &name2);
        if (rval != TPM2_RC_SUCCESS)
            return rval;
        // LOG_INFO("name2.size = %d", name2.size);
        // for (int ii = 0; ii < name2.size; ii++) {
        //     LOG_INFO("name2.byte-%d: %02x", ii, name2.name[ii]);
        // }
        rval = tpm_handle_to_name(tcti_context, handle3, &name3);
        if (rval != TPM2_RC_SUCCESS)
            return rval;
        // LOG_INFO("name3.size = %d", name3.size);
        // for (int ii = 0; ii < name3.size; ii++) {
        //     LOG_INFO("name3.byte-%d: %02x", ii, name3.name[ii]);
        // }
        rval = Tss2_Sys_GetCpBuffer(sysContext, &parametersSize, &startParams);
        LOG_INFO("command param size = %ld", parametersSize);
        if (rval != TPM2_RC_SUCCESS)
            return rval;
    } else {
        rval = Tss2_Sys_GetRpBuffer(sysContext, &parametersSize, &startParams);
        // LOG_INFO("response_parametersSize = %ld, Params:", parametersSize);
        // for (int i = 0; i < parametersSize; i++) {
        //     LOG_INFO("Byte-%d: %02x", i, startParams[i]);
        // }
        if (rval != TPM2_RC_SUCCESS)
            return rval;
    ...
    *(UINT32 *)hashInputPtr = cmdCode;
    ...
    if (hashInput.size > sizeof(hashInput.buffer))
        return TSS2_SYS_RC_INSUFFICIENT_BUFFER;
    LOG_INFO("hashInput.size = %d", hashInput.size);
