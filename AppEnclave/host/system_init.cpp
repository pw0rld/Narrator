...
void system_init()
{
	...
	if (ser->is_server_connected() && established_token)
	{
		...
		case SYSTEM_INIT_STATE:
		{
			...
			// }
			// else{
			// 	cout << "Start to updateCounter.And milliseconds time is " << ser->print_time() << endl;
			// 	ser->send_client_requests(STATE_INIT);
			// 	wait_count = MESSAGE_WAIT_COUNT;
			// 	if (PRINT_ATTESTATION_MESSAGES)
			// 	{
			// 		std::cout << "Peer (" << my_ip << ":" << my_port << ") sent state init request to server (" << ser->get_server_ip() << ":" << ser->get_server_port() << ")." << std::endl;
			// 	}
			// }
			// break;
		}
		...
		case SYSTEM_GET_STATE:
		{
			...
			// 	cout << "Start to updateCounter.And milliseconds time is " << milliseconds << endl;
	...
	if (system_state != SYSTEM_INIT_DONE || is_system_init != true)
	{
		system_init();
	}
	else
